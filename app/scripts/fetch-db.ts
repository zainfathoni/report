...
/**
 * Fetch the database from the Google Sheets
...
export async function fetchDatabase() {
  ...
  fs.writeFileSync(
    path.resolve(__dirname, "../data/report-db.json"),
...
(function fetchSenarai() {
  ...
  const spinner = ora(
    `${chalk.yellowBright("Fetching Report database...")}`,
  ).start();
  fetchDatabase()
    .then(() => {
      const end = `${toSecond(process.hrtime(start))} seconds`;
      spinner.succeed(
        `Fetching Report database is done in ${chalk.greenBright(end)}`,
      );
    })
    .catch((err) => {
      chalk.red(err);
